...
>
> 내가 기록한 식재료 가격 시계열을 이해하고, 그 데이터를 근거로 “지금 사도 되는지”까지 판정해 주는 AI 웹 서비스

## 평가자용 제출 요약

| 항목 | 제출 내용 | 바로가기 |
|---|---|---|
| 공개 서비스 | 모바일 중심 ‘따뜻한 장보기’ 화면 | Vercel 보안 로그인 완료 후 URL 확정 |
| 핵심 시연 | 추천 → 사진/바코드 → 장바구니 → 강사모드 | [`docs/시연순서_정리보고서.md`](docs/시연순서_정리보고서.md) |
| 팀미션 적합성 | 필수 산출물 및 AI 기술 요소 대조 | [`docs/제출보고서.md`](docs/제출보고서.md) |
| 평가 체크 | 기준별 구현·증거·검증 상태 | [`docs/평가기준_최종_체크리스트.md`](docs/평가기준_최종_체크리스트.md) |
| 최종 성과 | 문제·설계·구현·검증·한계·개선 계획 | [`docs/최종_결과보고서.md`](docs/최종_결과보고서.md) |
| 최종 제출 PDF | 제출보고서·체크리스트·시연·결과·적합성 통합본 | [`docs/제철밥상_플래너_2.0_최종제출보고서.pdf`](docs/제철밥상_플래너_2.0_최종제출보고서.pdf) |
| 발표 자료 | ‘제철밥상 플래너 2.0’ 12장 발표용 PPTX | [`presentation/제철밥상_플래너_2.0_최종발표자료.pptx`](presentation/제철밥상_플래너_2.0_최종발표자료.pptx) |

### 팀미션 최종 판정

**코드·문서 기준 제출 적합**입니다. 생성형 AI/AI Agent를 서비스 핵심에 두고, 다음 네 가지 요소를 실제 코드 흐름으로 연결했습니다. 최종 제출 전 Vercel 공개 URL만 확정하면 됩니다.

1. **AI Agent**: GPT가 11개 도구 중 필요한 도구를 선택하고 최대 3라운드 실행
2. **검색 증강(RAG 방식)**: 저장 가격·통계·품목 지식을 조회해 답변 컨텍스트로 주입
3. **Long-term Memory**: 대화 전체를 저장하고 대화 ID로 다시 불러와 이어서 질문
4. **자동화 워크플로우**: 데이터 요약 → 판정 → 도구 호출 → 답변 → 대화 저장 자동 처리

사진·바코드는 시장 방문 UX를 검증하기 위한 **사용자 확인형 프로토타입**입니다. 사진 미리보기와 최대 3개 후보 선택, 바코드 샘플 조회, 직접 입력 대체 흐름까지 시연할 수 있습니다. 운영용 비전 모델·상품 DB·OCR 연결은 후속 고도화 범위로 구분했습니다.

### 3분 평가 동선

1. 홈에서 서비스 문제와 1·2차 사용자를 설명합니다.
2. ‘밥상추천’에서 인원·시간·분위기에 맞는 메뉴를 생성합니다.
3. ‘시장담기’에서 사진 후보 확인 또는 바코드 숫자 입력을 시연합니다.
4. 장바구니 합계와 요리교실 강사모드의 1인 비용을 확인합니다.
5. 고급 화면에서 AI 채팅·가격 근거·대화 저장 구조를 설명합니다.
6. GitHub의 체크리스트에서 구현 코드와 남은 고도화 항목을 투명하게 제시합니다.

**바코드 시연 번호**: `8800000000003` 돼지고기 · `8800000000004` 고등어 · `8800000000005` 오징어

### 필수 산출물 대응

| PDF 요구 산출물 | 저장소 증거 | 상태 |
|---|---|---|
| 기획서 | README 1절, 기능설명서, 최종 결과보고서 | 충족 |
| 기능 요구사항 정의서 | [`docs/제출보고서.md`](docs/제출보고서.md)의 FR/NFR 표 | 충족 |
| AI 서비스 | `backend/app/services/chat_service.py`, `tools.py` | 충족 |
| GitHub 저장소 | 소스·문서·발표자료와 커밋 이력 | 충족 |
| 발표 자료 | `presentation/제철밥상_팀미션_시연발표.pptx` | 충족 |
| 서비스 데모 | 실행 가능한 프론트엔드 + 로컬/Render API 구조 | Vercel URL 확정 대기 |

> 정확성 원칙: 서버가 계산한 가격 판정과 근거를 AI 컨텍스트에 주입하고, AI는 그 근거 범위에서 설명합니다. 생성형 AI 사용 사실, 사용자 최종 확인, 사진 비저장 원칙도 화면에 표시합니다.
...
시드 데이터는 **12개 품목 × 730일 = 8,760개 레코드**로 구성됩니다. 날짜 범위와 실제 KAMIS 원자료의 중복·결측·단위는 운영 전 별도 검증이 필요합니다.
...
| 구분 | URL |
|---|---|
| 실행형 프론트엔드 | [제철밥상 플래너](https://jecheolbapsang-planner.youngdream.chatgpt.site) |
| Vercel 배포 | `frontend/` 폴더를 Root Directory로 지정해 배포 가능 |
| 백엔드 API | `https://<서비스명>.onrender.com` |
| Swagger UI | `https://<서비스명>.onrender.com/docs` |

현재 공개 프론트엔드는 정적 배포본입니다. 실시간 KAMIS 가격·AI 채팅·Firestore 기능은 Render 백엔드 URL과 환경변수를 연결한 뒤 활성화됩니다.
...
└── frontend/
    ...
    ├── simple.html                  # 일반 가정·시장 방문·요리교실 강사 중심 간편 화면
    ├── simple.css / simple.js       # 사진 후보 3개·바코드·장바구니·강사 비용 계산
...
| `GET` | `/api/market/current` | KAMIS 채소·축산·수산물 현재 소매가격 |
...
기존 AI 대시보드는 `frontend/index.html`에서 실행하고, 사용자 테스트용 따뜻한 밥상 화면은 `frontend/simple.html`에서 실행합니다. 정적 서버 예시는 다음과 같습니다.
...
브라우저에서 `http://localhost:5173/simple.html`을 열면 됩니다. 백엔드가 꺼져 있어도 추천·시장 담기·장바구니·강사모드가 fixture/localStorage로 시연됩니다.

### 구현된 사용자 흐름

- 일반 가정 사용자: 오늘의 밥상 추천 → 시장에서 사진/바코드/직접입력 → 재료 확인 → 장바구니 저장
- 요리교실 강사: 수업 인원 입력 → 장바구니 기반 1인 예상비용 → 장보기 목록 공유
- 시장 기능: 채소·과일 사진 미리보기 및 후보 확인, 바코드 숫자/지원 브라우저 카메라 훅, 영수증 사진 보조 보관

사진의 AI 품목 분류와 영수증 OCR은 현재 “사용자 확인이 포함된 1차 UX”로 구현했습니다. 운영 단계에서는 비전 모델·OCR API를 `/api/market/identify`와 `/api/market/receipt`로 연결하고, 인식 결과는 반드시 사용자가 확정하도록 확장합니다.

## 제출 전 필수 점검

- 실데이터는 날짜·품목·단위가 유일한지 검증하고, 현재값/평균/최저값의 정의를 README와 화면에 동일하게 표시합니다.
- `.env`, `serviceAccountKey.json`, API 키가 배포물·증빙 이미지·Git 이력에 포함되지 않도록 합니다.
- 팀미션의 AI Agent·RAG·Long-term Memory는 기존 백엔드의 채팅/도구/대화 저장 구조를 활용하되, 사용자별 기억과 검색 근거를 시연 자료에 명시합니다.
